      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            ...
            <form action="{{url('/search-product')}}" class="nav-link mt-2 search col-12 col-md-6 mb-3" method="GET">
              @csrf
                            ...
                            <tr>
                                <td>
                                    @if(in_array(strtolower(pathinfo($product->image, PATHINFO_EXTENSION)), ['mp4', 'webm', 'ogg', 'mov', 'avi']))
                                        <video src="products_images/{{$product->image}}" style="max-width: 60px; border-radius: 5px;" muted autoplay loop></video>
                                    @else
                                        <img src="products_images/{{$product->image}}" alt="image" />
                                    @endif
                                </td>
                                <td>{{$product->category}}</td>
                                <td style="color: orange">Rs. {{ ltrim($product->price, '$') }}</td>
                                <td class="text-success">Rs. {{ ltrim($product->discount_price, '$') }} <i class="mdi mdi-arrow-up"></i></td>
                            ...
                            <tr>
                              <td colspan="16">
                                <div class="text-center">
                                  <img style="width: 25%;height: 25%;" src="/user/assets/imgs/no-search-result.jpg" alt="no-search-result">
